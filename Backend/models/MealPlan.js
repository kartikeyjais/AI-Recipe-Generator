...
static async create(userId , mealData){
 ...
 static async getWeeklyPlan(userId , weekStarDate) {
 
    const endDate = new Date(weekStarDate);
    endDate.setDate(endDate.getDate()+6);

    return await this.findByDateRange(userId , weekStarDate , endDate);

 }

 // get upcoming meal next 7 days
 ...
 static async getUpcoming(userId , limit = 5){
      ...
      const result = await db.query(
        `SELECT mp.*, r.name as recipe_name, r.image_url 
         FROM meal_plans mp 
         JOIN recipes r ON mp.recipes_id = r.id
         WHERE mp.user_id = $1 
 ...
 // Delete meal plan
  ...
  static async getStats(userId){
       ...
       `SELECT 
        ...
        COUNT(*) FILTER (WHERE meal_date >= CURRENT_DATE AND meal_date < CURRENT_DATE + INTERVEL '7 days') as this_week_count
       FROM meal_plans
       WHERE user_id = $1`,
                   [userId]
       );
